  ...
  "Calling .preserve" should {
    "preserve known pages stored as JsObjects" in {
      ...
      val trimmedAnswers: UserAnswers           = UserAnswers(completeUserAnswers.id)
        .set(KnownEoriDetailsPage, completeSubmission.knownDetails)
        .success
        .value
        .set(UserTypePage, completeSubmission.userType)
        .success
        .value
    ...
    "preserve known pages stored as simple JsValues e.g. Strings or Booleans" in {
      ...
      val trimmedAnswers: UserAnswers           = UserAnswers(completeUserAnswers.id)
        .set(TraderAddressCorrectPage, completeSubmission.traderAddressCorrect)
        .success
        .value
        .set(MoreInformationPage, completeSubmission.additionalInfo.getOrElse("Additional information"))
        .success
        .value
    ...
    "preserve known pages stored at a path deeper than 1 level" in {
      ...
      val trimmedAnswers: UserAnswers           = UserAnswers(completeUserAnswers.id)
        .set(EnterCustomsProcedureCodePage, completeSubmission.originalCpc.getOrElse("cpcError"))
        .success
        .value
    ...
    "preserve known pages stored as JsArrays" in {
      ...
      val trimmedAnswers: UserAnswers           = UserAnswers(completeUserAnswers.id)
        .set(UnderpaymentReasonsPage, completeSubmission.amendedItems.get)
        .success
        .value
