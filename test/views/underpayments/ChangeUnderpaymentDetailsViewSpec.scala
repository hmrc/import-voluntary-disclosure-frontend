  ...
  "Rendering the ChangeUnderpaymentDetailsView page" when {
    "no errors exist" should {
      ...
      "have correct heading" in {
        document.select("h1").text mustBe ChangeUnderpaymentDetailsMessages.underpaymentTypeContent(
          underpaymentType
        ).title
      }
  ...
  "The Underpayment Reason Amendment page" when {
    ...
    def checkContent(underpaymentType: String): Unit = {
      ...
      s"rendered for type $underpaymentType for changing of type" should {
        ...
        "have the correct remove link heading" in {
          elementText("#remove-link") mustBe ChangeUnderpaymentDetailsMessages.underpaymentTypeContent(
            underpaymentType
          ).remove
