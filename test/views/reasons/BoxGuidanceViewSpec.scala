...
class BoxGuidanceViewSpec extends ViewBaseSpec with BaseMessages {
  ...
  "Rendering the box guidance page" when {
    "no errors exist" should {
      lazy val view: Html = injectedView(Call("GET", "backLink"), displayBackLink = true)(fakeRequest, messages)
    ...
    "when in check mode" should {
      lazy val view: Html = injectedView(Call("GET", "backLink"), displayBackLink = false)(fakeRequest, messages)
      lazy implicit val document: Document = Jsoup.parse(view.body)
