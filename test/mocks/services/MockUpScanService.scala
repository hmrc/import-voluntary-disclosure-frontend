...
trait MockUpScanService extends MockFactory {
  ...
  object MockedUpScanService {
    def initiateNewJourney(response: Future[UpScanInitiateResponse]): CallHandler[Future[UpScanInitiateResponse]] =
      (mockUpScanService
        .initiateNewJourney()(_: ExecutionContext, _: HeaderCarrier))
        .expects(*, *)
        .returns(response)

    def initiateBulkJourney(response: Future[UpScanInitiateResponse]): CallHandler[Future[UpScanInitiateResponse]] =
      (mockUpScanService
        .initiateBulkJourney()(_: ExecutionContext, _: HeaderCarrier))
        .expects(*, *)
        .returns(response)
    ...
    def initiateAuthorityJourney(
      ...
      (mockUpScanService
        .initiateAuthorityJourney(_: String, _: String)(_: ExecutionContext, _: HeaderCarrier))
        .expects(*, *, *, *)
        .returns(response)
    ...
    def initiateSupportingDocJourney(
      ...
      (mockUpScanService
        .initiateSupportingDocJourney()(_: ExecutionContext, _: HeaderCarrier))
        .expects(*, *)
        .returns(response)
    ...
    def initiateCancelCaseJourney(
      ...
      (mockUpScanService
        .initiateCancelCaseJourney()(_: ExecutionContext, _: HeaderCarrier))
        .expects(*, *)
        .returns(response)
