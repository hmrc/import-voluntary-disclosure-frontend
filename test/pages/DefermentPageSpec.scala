...
class DefermentPageSpec extends WordSpec with MustMatchers {
  "DefermentPage" when {
    "choosing not to pay by deferment" should {
      "remove answers relating to deferment" in {
        ...
        val answersAfterCleanUp = DefermentPage.cleanup(Some(payByDeferment), answers).success.value
