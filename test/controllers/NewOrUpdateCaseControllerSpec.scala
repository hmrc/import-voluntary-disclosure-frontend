...
import pages.NewOrUpdateCasePage
...
class NewOrUpdateCaseControllerSpec extends ControllerSpecBase {

  trait Test extends MockSessionRepository {
    private lazy val view: NewOrUpdateCaseView = app.injector.instanceOf[NewOrUpdateCaseView]
    ...
    val form: NewOrUpdateCaseFormProvider = formProvider

    MockedSessionRepository.set(Future.successful(true))

    lazy val controller = new NewOrUpdateCaseController(authenticatedAction, dataRetrievalAction, dataRequiredAction,
  ...
  "GET onLoad" should {
    "return OK" in new Test {
      override val userAnswers: Option[UserAnswers] = Some(UserAnswers("some-cred-id")
        .set(NewOrUpdateCasePage, true).success.value)
    ...
    "payload contains valid data" should {
      ...
      "return a SEE OTHER then current saved value is true and submitted is true" in new Test {
        override val userAnswers: Option[UserAnswers] = Some(
          UserAnswers("some-cred-id").set(NewOrUpdateCasePage, true).success.value
        )
      ...
      "return a SEE OTHER when there's no current saved value and submitted is false" in new Test {
        ...
        lazy val result: Future[Result] = controller.onSubmit()(request)
        status(result) mustBe Status.SEE_OTHER
        redirectLocation(result) mustBe Some(controllers.routes.NewOrUpdateCaseController.onLoad().url)
      }
