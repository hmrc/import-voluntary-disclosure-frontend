  ...
  "POST onSubmit" when {
    "payload contains valid data" should {
      ...
      "return a SEE OTHER response when false" in new Test {
        override val userAnswers: Option[UserAnswers]        = Some(
          UserAnswers("some-cred-id")
            .set(UnderpaymentDetailSummaryPage, Seq(UnderpaymentDetail("B00", 50, 60)))
            .success
            .value
        )
      ...
      "redirect to Details Underpayment Summary page" in new Test {
        override val userAnswers: Option[UserAnswers]        = Some(
          UserAnswers("credId")
            ...
            .success
            .value
            .set(UnderpaymentDetailSummaryPage, Seq(UnderpaymentDetail("A35", 50, 60)))
            .success
            .value
        )
      ...
      "redirect to Details Underpayment Start page" in new Test {
        override val userAnswers: Option[UserAnswers]        = Some(
          UserAnswers("credId")
            .set(UnderpaymentDetailSummaryPage, Seq.empty)
            .success
            .value
        )
        val request: FakeRequest[AnyContentAsFormUrlEncoded] = fakeRequest.withFormUrlEncodedBody("value" -> "true")
