...
class UploadSupportingDocumentationControllerSpec extends ControllerSpecBase with BeforeAndAfterEach {
  ...
  def controller(ua: Option[UserAnswers] = userAnswers): UploadSupportingDocumentationController = {
  ...
  override def beforeEach(): Unit = {
    ...
    when(mockUpScanService.initiateSupportingDocJourney()(any(), any())).thenReturn(
      Future.successful(
  ...
  "backLink" should {
    ...
    "return link to Do You Need To Send Us More Documentation" in {
      val ua: UserAnswers = UserAnswers("credId")
      val result: Call    = controller().backLink()(dataRequest.copy(userAnswers = ua))
