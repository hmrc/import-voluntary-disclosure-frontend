  ...
  "POST onSubmit" when {
    "payload contains valid data" should {
      ...
      "redirect to Reason Underpayment Summary page when not removing other reason" in new Test {
        override val userAnswers: Option[UserAnswers]        = Some(
          UserAnswers("credId")
            .set(UnderpaymentReasonsPage, Seq(underpaymentReason(boxNumber = BoxNumber.OtherItem, itemNumber = 0)))
            .success
            .value
            .set(
              ChangeUnderpaymentReasonPage,
      ...
      "redirect to Box Guidance page" in new Test {
        override val userAnswers: Option[UserAnswers]        = Some(
          UserAnswers("credId")
            .set(UnderpaymentReasonsPage, Seq(underpaymentReason(boxNumber = BoxNumber.Box35, itemNumber = 1)))
            .success
            .value
            .set(
              ChangeUnderpaymentReasonPage,
