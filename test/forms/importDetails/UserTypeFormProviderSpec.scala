...
class UserTypeFormProviderSpec extends FormSpecBase {
  ...
  "Binding a form with invalid data" when {
    ...
    "the no value selected" should {
      ...
      "have an error with the correct message" in {
        form.errors.head.message mustBe "userType.error.required"
      }
    }
  }
  ...
  "Binding a form with valid data" should {
    ...
    val data = Map("value" -> "importer")
    val form = new UserTypeFormProvider()().bind(data)
    ...
    "generate the correct model" in {
      form.value mustBe Some(UserType.Importer)
    }
  }
