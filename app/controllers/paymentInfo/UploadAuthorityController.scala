...
class UploadAuthorityController @Inject() (
  ...
  def upscanResponseHandler(
    ...
    val successRoute =
      Redirect(
        controllers.paymentInfo.routes.UploadAuthorityController
          .uploadProgress(dutyType, dan, key.getOrElse("this will never be used"))
      )
  ...
  def uploadProgress(dutyType: SelectedDutyType, dan: String, key: String): Action[AnyContent] =
    (identify andThen getData andThen requireData).async { implicit request =>
      ...
      val updateFilesList: FileUpload => Seq[UploadAuthority] = { file =>
        ...
        request.userAnswers
          .get(UploadAuthorityPage)
          .getOrElse(Seq.empty)
          .filterNot(_.dutyType == dutyType) :+ newAuthority
      }
  ...
  def onSuccess(dutyType: SelectedDutyType, dan: String): Action[AnyContent] =
    (identify andThen getData andThen requireData).async { implicit request =>
      ...
      val filename = request.userAnswers
        .get(UploadAuthorityPage)
        .getOrElse(Seq.empty)
