...
@Singleton
class UpScanController @Inject()(val mcc: MessagesControllerComponents,
  ...
  def initiateFileUpload: Action[AnyContent] = (identify andThen getData).async { implicit request =>
      upScanService.initiateNewJourney().map { response =>
        Ok(uploadFileView(response))
          .addingToSession(SessionKeys.upscanReference -> response.reference.value)
      }
  }
