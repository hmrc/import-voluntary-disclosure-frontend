...
@Singleton
class CancelCaseUploadSupportingDocumentationController @Inject()(identify: IdentifierAction,
  ...
  def upscanResponseHandler(key: Option[String] = None,
    ...
    val successRoute = Redirect(
      controllers.cancelCase.routes.CancelCaseUploadSupportingDocumentationController.uploadProgress(key.getOrElse("this will never be used"))
    )
  ...
  def uploadProgress(key: String): Action[AnyContent] = (identify andThen getData andThen requireData).async { implicit request =>
    val uploadCompleteRoute = Redirect(controllers.cancelCase.routes.CancelCaseUploadSupportingDocumentationSummaryController.onLoad())
    val uploadFailedRoute = Redirect(controllers.cancelCase.routes.CancelCaseUploadSupportingDocumentationController.onLoad())
    val uploadInProgressRoute = Ok(
      progressView(
        key = key,
        action = controllers.cancelCase.routes.CancelCaseUploadSupportingDocumentationController.uploadProgress(key).url
      )
    )
