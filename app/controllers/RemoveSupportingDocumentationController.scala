  ...
  def onLoad(index: Index): Action[AnyContent] = (identify andThen getData andThen requireData).async {
    implicit request =>
      request.userAnswers.get(UploadSupportingDocumentationPage) match {
        ...
        case _ => Future.successful(Redirect(controllers.routes.UploadSupportingDocumentationController.onLoad()))
      }
  }
        ...
        value => {
          if (value) {
            for {
              ...
              Redirect(controllers.routes.UploadSupportingDocumentationSummaryController.onLoad())
          } else {
