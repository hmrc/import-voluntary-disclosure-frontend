...
class UploadSupportingDocumentationController @Inject()(identify: IdentifierAction,
  ...
  def upscanResponseHandler(key: Option[String] = None,
    ...
    val errorRoute = Redirect(controllers.routes.UploadSupportingDocumentationController.onLoad())
    val successRoute = Redirect(controllers.routes.UploadSupportingDocumentationController.uploadProgress(key.getOrElse("this will never be used")))
