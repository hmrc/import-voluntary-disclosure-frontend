  ...
  private[mappings] def numericFormatter(
    ...
    new Formatter[BigDecimal] {
      ...
      override def bind(key: String, data: Map[String, String]) = {
        ...
        val numericFormatter =
          stringFormatter(requiredKey).bind(key, data).right.map(_.replace(",", "")).right.map(_.replaceAll("\\s", ""))

        val currencyFormatter = numericFormatter.right.map(_.replace("£", ""))
        ...
        if (isCurrency) {
          currencyFormatter.right.flatMap(validation)
        } else {
          numericFormatter.right.flatMap(validation)
        }
      }
