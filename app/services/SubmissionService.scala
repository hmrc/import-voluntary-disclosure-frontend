...
@Singleton
class SubmissionService @Inject() (ivdSubmissionConnector: IvdSubmissionConnector, auditService: AuditService) {
  ...
  def createCase(implicit
    ...
    hc: HeaderCarrier,
    ec: ExecutionContext
  ): Future[Either[ErrorModel, SubmissionResponse]] = {
  ...
  private[services] def buildDefermentDetails(data: SubmissionData): JsObject = {
    if (data.paymentByDeferment) {
      (
        data.defermentType,
        data.defermentAccountNumber,
        data.additionalDefermentAccountNumber,
        data.additionalDefermentType,
        data.splitDeferment
      ) match {
